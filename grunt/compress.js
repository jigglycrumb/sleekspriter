module.exports = {
  ...
  desktopWin32: {
    ...
    files: [{expand: true, cwd: 'dist/desktop/windows/<%= package.name %>/win32/', src: '**/*', dest: '<%= package.name %>/'}],
  },
  desktopWin64: {
    options: {
      archive: 'release/<%= package.name %>-<%= package.version %>-win64.zip',
    },
    files: [{expand: true, cwd: 'dist/desktop/windows/<%= package.name %>/win64/', src: '**/*', dest: '<%= package.name %>/'}],
  },
  /*
  desktopLinux32: {
    options: {
