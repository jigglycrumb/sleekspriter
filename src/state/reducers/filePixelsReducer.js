...
function filePixelsReducer(state = initialState.file.pixels, action) {
  ...
  case "FRAME_DUPLICATE": {
    ...
    let layerMap = {};
    layers.map((layer, index) => { layerMap[layer.id] = newLayers[index].id; });
  ...
  case "PIXELS_MOVE": {
    ...
    const bounds = createBounds(size);
    const newPixels = manipulatePixels(pixels, movePixel.bind(this, distance, bounds));
